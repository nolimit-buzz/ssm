...
const SingleNewsPage: React.FC<SingleNewsPageProps> = ({ article, onNavigate, onReadArticle, onNavigateToCategory, isDarkMode, onToggleTheme }) => {
  ...
  // Mock "Related News" Data (Subset for Footer)
  const RELATED_NEWS = [
    {
      id: 2,
      category: "Milestones",
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${bgClass}`}>
      ...
      <article className="px-6">
        <div className={`max-w-3xl mx-auto ${textSecondary} text-lg md:text-xl leading-relaxed space-y-8 font-medium`}>
          
          <p className="first-letter:text-6xl first-letter:font-black first-letter:float-left first-letter:mr-3 first-letter:text-emerald-600">
